  ...
  static {
    RULES.put(
        ...
        new Rule<CharSequence>() {
          ...
          private final int[] mode18Calculations = {
            7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2
          };

          private final char[] mode18Suffixes = {
          ...
          public boolean isValid(
            ...
            String birthday =
                new StringBuilder(target.substring(6, 10))
                    .append('-')
            ...
            for (int count = 17; count >= 0; count--) {
              calcValue +=
                  Math.pow(2, count)
                      % 11
                      * Integer.parseInt(String.valueOf(target.charAt(17 - count)), 11);
            }
          ...
          private String upgrade(String input) {
            if (this.mode15.matcher(input).matches()) {
              ...
              StringBuilder output =
                  new StringBuilder(input.substring(0, 6))
                      .append("19")
                      .append(input.substring(6, input.length()));

              for (int count = 0; count < input.length(); count++) {
                calcValue +=
                    Integer.parseInt(input.substring(count, count + 1))
                        * this.mode18Calculations[count];
              }
