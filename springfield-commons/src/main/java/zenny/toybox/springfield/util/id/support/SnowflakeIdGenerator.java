 ...
 *
 * <p>Note that for the snowflake algorithm, the key to ensure that the ID generated by it is
 * globally unique is: 1. Time; 2. Identifier and sub-identifier.
 *
 * <p>Therefore, when using this algorithm, it is important to ensure that there are time service
 * servers in the server cluster, and locate the unique combination of identifier and sub-identifier
...
public class SnowflakeIdGenerator implements IdGenerator<Long> {
  ...
  public SnowflakeIdGenerator(
      long epoch, @Nullable IdentifierLookup lookuper, @Nullable IdentifierLookup subLookuper) {
    this(
        epoch,
  ...
  public SnowflakeIdGenerator(long epoch, long identifier, long subIdentifier) {
    Assert.isTrue(
        epoch >= 0 && epoch <= this.getCurrentTimestamp(),
        "Epcho can't be less than 0 or greater than the current time");
    Assert.isTrue(
        identifier >= 0 && identifier <= MAX_IDENTIFIER,
        "Identifier can't be less than 0 or greater than " + MAX_IDENTIFIER);
    Assert.isTrue(
        subIdentifier >= 0 && subIdentifier <= MAX_SUB_IDENTIFIER,
    ...
    if (this.logger.isInfoEnabled()) {
      this.logger.info(
          "SnowflakeIdGenerator has been created. The epoch is ["
              + this.epoch
              + "], the identifier is ["
              + this.identifier
              + "], and the sub-identifier is ["
              + this.subIdentifier
              + "]");
    }
  }
  ...
  protected long tilNextMillis(long lastTimestamp) {
    ...
    while (mill <= lastTimestamp) {
      if (alerted == false) {
        alerted = true;
        this.alerter.accept(
            lastTimestamp, this.logger); // Do something else and slow down the spin.
      }
