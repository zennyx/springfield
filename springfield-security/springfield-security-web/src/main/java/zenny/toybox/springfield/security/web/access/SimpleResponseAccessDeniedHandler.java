...
public class SimpleResponseAccessDeniedHandler implements AccessDeniedHandler {
  ...
  private final BiFunction<
          HttpServletRequest, HttpServletResponse, Function<AccessDeniedException, ResponseContent>>
      contentSupplier;
  ...
  public SimpleResponseAccessDeniedHandler(String errorPage) {
    this(
        errorPage,
        new DefaultResponseStrategy(HttpStatus.FORBIDDEN),
        (request, response) -> {
          return (error) -> {
  ...
  public SimpleResponseAccessDeniedHandler(
      ...
      BiFunction<
              HttpServletRequest,
              HttpServletResponse,
              Function<AccessDeniedException, ResponseContent>>
          contentSupplier) {
    Assert.isTrue(
        StringUtils.hasText(errorPage) && UrlUtils.isValidRedirectUrl(errorPage),
