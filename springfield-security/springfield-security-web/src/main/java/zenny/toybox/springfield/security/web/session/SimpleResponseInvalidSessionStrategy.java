...
public class SimpleResponseInvalidSessionStrategy implements InvalidSessionStrategy {
  ...
  private final BiFunction<HttpServletRequest, HttpServletResponse, ResponseContent>
      contentSupplier;
  ...
  public SimpleResponseInvalidSessionStrategy(String invalidUrl, boolean createNewSession) {
    this(
        invalidUrl,
        createNewSession,
        new DefaultResponseStrategy(HttpStatus.UNAUTHORIZED),
        (request, response) -> {
  ...
  public SimpleResponseInvalidSessionStrategy(
      String invalidUrl,
      boolean createNewSession,
      ResponseStrategy responseStrategy,
      BiFunction<HttpServletRequest, HttpServletResponse, ResponseContent> contentSupplier) {
    Assert.isTrue(
        StringUtils.hasText(invalidUrl) && UrlUtils.isValidRedirectUrl(invalidUrl),
  ...
  public void onInvalidSessionDetected(HttpServletRequest request, HttpServletResponse response)
    ...
    this.responseStrategy.response(
        request,
        response,
        this.contentSupplier.apply(request, response),
        () -> {
